  ...
  return (
    <SafeAreaView className="flex-1 bg-wood-900" edges={['top', 'left', 'right']}>
      <View className="flex-1 px-6 py-8">
        <Text className="font-serif text-2xl text-cream">Microphone</Text>
        <Text className="mt-3 font-sans text-sm leading-6 text-muted-brown">
          Harmoniq listens only to score your placement phrases. Nothing is uploaded until you run analysis on a song later.
        </Text>

        {status === 'denied' ? (
          <View className="mt-6 rounded-lg border border-danger/35 bg-danger/10 px-3 py-3">
            <Text className="font-sans text-sm text-cream">Microphone access is off for this app.</Text>
            <Text className="mt-2 font-sans text-xs text-muted-brown">
              Enable the mic in system settings, then tap Refresh below.
            </Text>
            <Pressable
              onPress={openSettings}
              className="mt-3 self-start rounded-md border border-amber-accent/50 px-3 py-2"
        ...
        <Pressable
          ...
          className="mt-10 rounded-lg border border-amber-accent/60 bg-wood-800/80 px-4 py-3 disabled:opacity-35"
          accessibilityRole="button"
        >
          <Text className="text-center font-sans-medium text-amber-light">Next: placement phrases</Text>
        </Pressable>

        <Pressable onPress={() => router.back()} className="mt-6 self-start px-1 py-2" accessibilityRole="button">
