...
export type ScoreViewerProps = {
  ...
  onCursorPositionUpdate?: (positionMs: number) => void
  onBeatEvent?: (beat: number) => void
...
export const ScoreViewer = forwardRef<AlphaTabSurfaceRef, ScoreViewerProps>(
  function ScoreViewer(
    ...
    useEffect(() => {
      ...
      ;(async () => {
        try {
          const harnessAsset = Asset.fromModule(HARNESS_HTML)
          await harnessAsset.downloadAsync()
          const uri = harnessAsset.localUri ?? harnessAsset.uri
          if (!cancelled) {
    ...
    }, [audioSrc, harnessReady, musicXml, postInbound, transposeSemitones, renderPreset, soundFontProfile])
    ...
    const onMessage = useCallback(
      (e: WebViewMessageEvent) => {
        ...
        if (msg.type === 'position') {
          ...
          getPositionResolverRef.current = null
          onCursorPositionUpdate?.(msg.positionMs)
          return
        }
      ...
      <View className="min-h-[220px] flex-1 overflow-hidden rounded-xl border border-wood-600/45 bg-ivory" style={style}>
        {harnessError ? (
          <View className="border-b border-amber-accent/30 bg-amber-accent/10 px-3 py-2">
            ...
            <Text className="mt-2 self-start font-sans-medium text-xs text-amber-accent underline" onPress={onRetry}>
              Reload harness
            </Text>
          </View>
        ) : null}
